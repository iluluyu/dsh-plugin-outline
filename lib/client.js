 ...
 *
 * Performance profile (scroll path costs nothing observable; refresh pays the
 * DOM reads so streaming doesn't have to):
 * - scroll tracking reads ONE rect (the container) and binary-searches cached
 *   turn offsets; the cache refills only on content mutation, host swap or
 *   window resize — never per scroll frame;
 * - turn snippets read textContent (not layout-aware innerText) and row objects
 *   are reused while their DOM node stays mounted, so an unchanged rail
 *   re-renders nothing between mutations;
 * - the fallback div-walk for scroller detection runs once per host element and
 *   is memoized; the 2s watchdog only scans when the plugin is off-chat.
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const { useCallback, useEffect, useLayoutEffect, useRef, useState, memo } = React;
		...
		/** Scroller-detection memo: the div walk is the one genuinely costly
		   probe (getComputedStyle per div), so it runs once per host element and
		   is re-validated by isConnected on every later call. */
		const scrollMemo = { host: null, el: null };

		/** Locate the scrolling element inside the conversation host. */
		...
		/** First line of a turn, whitespace-flattened, for the outline row.
		   textContent deliberately: innerText is layout-aware and would force a
		   synchronous reflow per turn on every refresh while the host React tree
		   streams — the snippet is whitespace-flattened and truncated anyway, so
		   layout awareness buys nothing here. */
		function snippet(el) {
		...
		/** One outline row. Memoized and hoisted OUTSIDE the nav component so
		   its type is stable across renders — an `active` move then re-renders
		   only the two rows whose .ol-on flips, not the rail. The click handler
		   is passed as a stable callback + index so memoization actually holds. */
		const Row = memo(function Row({ i, on, text, onJump }) {
			return h("div", {
				className: on ? "ol-row ol-on" : "ol-row",
				title: text,
				onClick: () => { onJump(i); },
			},
				h("span", { className: "ol-txt" }, text),
		...
		function OutlineNav() {
			...
			const scrollElRef = useRef(null); // element the scroll listener is bound to
			const rafPending = useRef(false);
			// Geometric memo: CONTENT offset of each turn (on-screen top minus
			// scroller top plus the scrollTop at measure time — invariant under
			// scrolling), refilled only on content mutation, host swap or resize —
			// so scroll tracking costs one container rect + a binary search per
			// frame instead of N rects.
			const topsRef = useRef([]);
			// Row identity memo: lets refresh() reuse row objects while their turn's
			// DOM node stays mounted (streaming swaps nodes in, not text).
			...
			const track = useCallback(() => {
				...
				// Cached tops are CONTENT offsets (invariant under scrolling); the probe
				// line is scrollTop + 35% of the viewport slice, in the same coordinates.
				// (scrollTop of a non-scrolling host stays 0, which is exactly right
				// when an ancestor is the real scroller — the offset never changes.)
				const probe = parts.scrollEl.scrollTop
					+ Math.min(scRect.height, window.innerHeight) * 0.35;
				const tops = topsRef.current;
				if (tops.length !== parts.users.length) return; // stale: refresh refills
				// Same result as walking every turn rect, but against cached offsets:
				// tops is sorted ascending, so binary-search the last top <= probe.
			...
			useEffect(() => {
				...
				const refresh = () => {
					...
					if (parts === null) {
						// Functional reset: identical empty state re-renders nothing.
						setTurns((cur) => cur.length === 0 ? cur : []);
						return;
					}
					setDark(detectTheme(parts.scrollEl));
					// Reuse row objects whose turn is unchanged (same DOM node AND same
					// flattened text — the host may edit a turn's text in place). While
					// nothing moved, React re-renders nothing; snippet() reads only
					// textContent, which costs ~0.3µs per turn.
					const prev = new Map(rowsRef.current.map((r) => [r.key, r]));
					const next = parts.users.map((el, i) => {
						const key = el.dataset.chatFlowKey ?? String(i);
						const text = snippet(el);
						const old = prev.get(key);
						return old !== undefined && old.el === el && old.text === text
							? old
							: { key, el, text };
					});
					...
					// Refill the geometric memo now, while refresh already touches layout.
					// Stored as content offsets: on-screen top minus scroller top plus the
				// scrollTop at measure time — invariant under subsequent scrolling.
				...
				const onResize = () => {
					// A resize storm reflows geometry anyway: refill the offset memo
					// (content coordinates), then track, rAF-gated exactly like scrolling.
					const parts = partsRef.current;
					if (parts === null) { topsRef.current = []; }
					else {
						const sc = parts.scrollEl;
						const scTop = sc.getBoundingClientRect().top;
						const x0 = sc.scrollTop;
				...
				const interval = setInterval(() => {
					// Off-chat or host swapped out: re-discover. The rebind is
					// idempotent (attachScroll skips when already bound to this
					// element) and also happens inside refresh(), so a host swap
					// picked up by the MutationObserver rebinds immediately.
					const parts = partsRef.current;
			...
			return h("div", {
				id: NAV_ID,
				ref: rootRef,
				className: [open && "ol-open", dark && "ol-dark", clipped && "ol-clipped"]
					.filter(Boolean).join(" "),
